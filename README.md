...
Boshqa maydonlar (`JWT_SECRET`, `SEED_ADMIN_EMAIL`, `SEED_ADMIN_PASSWORD` va h.k.) ixtiyoriy — xohlasangiz
...
Bu admin foydalanuvchi (`.env`dagi `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD`, standart:
`admin@sotuv.uz` / `admin12345`), 3 ta kategoriya va 4 ta namuna mahsulot (tariflari, demo materiallari,
bitta sharh bilan) yaratadi.
...
1. Seed skriptidagi admin email/parol bilan kiring
...
- **Email xabarnomalari**: `.env`dagi `SMTP_*` va `ADMIN_NOTIFY_EMAIL` maydonlari bo'sh qoldirilsa, xabar
  yuborish jim o'tkazib yuboriladi (xatolik bermaydi).
- Standart admin parolini production muhitida albatta almashtiring.
